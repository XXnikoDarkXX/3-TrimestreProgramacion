...
public class MiMysql
{
    ...
    public static DefaultTableModel ConsultaPrestamo2()
    {
		...
		try
		{
			...
			Statement s = conn.createStatement();
			ResultSet rs = s.executeQuery("SELECT Prestamos.id_prestamo, Prestamos.id_libro,Prestamos.id_revista,Prestamos.NSocio,Prestamos.Fecha_inicio,Prestamos.Fecha_fin,Prestamos.Devuelto,Prestamos.Consulta,Usuarios.Nombre,Usuarios.Apellidos FROM Prestamos join Usuarios on Prestamos.Nsocio = Usuarios.Nsocio WHERE Devuelto = FALSE AND Fecha_fin < CURRENT_DATE()");
			//System.out.println(Llamada);
			// Creamos las columnas.
		    modelo.addColumn("ID Prestamo");
			modelo.addColumn("ID Libro");
