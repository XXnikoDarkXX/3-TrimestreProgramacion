...
public class VentanaMensajeSalida extends JFrame
{
	static final int xOffset = 30, yOffset = 30;
	public VentanaMensajeSalida(String id)
	{
		super("Mensaje Nuevo");
	...
	class MensajeUsuario extends JPanel implements ActionListener
	{
		...
		JComboBox textTecnico = new JComboBox();
		...
		public  MensajeUsuario(String id) 
		{
			...
			textRemitente = new JTextField(id,25);
			...
			//Etiqueta para Fecha
			labelTecnico = new JLabel("¿Para un tecnico?");
	        labelTecnico.setLabelFor(textTecnico);
	        ...
	        textTecnico.addItem("NO");
	        textTecnico.addItem("SI");
	        textTecnico.addActionListener(this);
	        textTecnico.setActionCommand("TECNICO");
			...
			JLabel[] labels = {labelRemitente, labelReceptor, labelAsunto};
		    JTextField[] textFields = {textRemitente, textReceptor, textAsunto};
			...
			textControlsPane.add(labelTecnico, c);

		    c.anchor = GridBagConstraints.WEST;
			c.gridwidth = GridBagConstraints.REMAINDER;     //end row
			c.fill = GridBagConstraints.NONE;
			c.weightx = 1.0;
			textControlsPane.add(textTecnico, c);
		...
		protected ImageIcon createImageIcon(String path) {
			...
			{
				String receptor = "2";
				if ("TECNICO".equals(e.getActionCommand()))
				{
					if (textTecnico.getSelectedItem().toString().compareTo("SI") == 0)
					{
						textReceptor.setText("");
						textReceptor.setEnabled(false);
						receptor = "2";
					}
				}
				else if ("ENVIAR".equals(e.getActionCommand()))
				{
					MiMysql enviaMensaje = new MiMysql();
					if (textReceptor.getText().compareTo("") != 0) receptor = textReceptor.getText();
					String envioDatos = textRemitente.getText() + "," +
										receptor 				+ ",'" +
										textAsunto.getText() 	+"','" +
										textMensaje.getText() 	+ "','" +
										enviaMensaje.dame_fecha("",false) +"');";

					
					enviaMensaje.LlamadaDB("INSERT INTO Buzon (NoNuevo,id_remitente,id_receptor,Asunto,Mensaje,Fecha_Envio) VALUES (FALSE,"
											+ envioDatos);
					enviaMensaje.LlamadaDB("INSERT INTO Buzon2 (id_remitente,id_receptor,Asunto,Mensaje,Fecha_Envio) VALUES ("
							+ envioDatos);
					JOptionPane.showMessageDialog(textControlsPane, "El mensaje se ha enviado con exito", "Prestamo exitoso", JOptionPane.INFORMATION_MESSAGE);
					//System.out.println("INSERT INTO Buzon (NoNuevo,id_remitente,id_receptor,Asunto,Mensaje,Fecha_Envio) VALUES (FALSE,"+envioDatos);
				}
			}
	}
}
